...
module.exports = {
	...
	calculate: async (req, pre_stub, total_hours = 0, i_index) => {
		...
		var away_date = isset(payload.away_date) ? payload.away_date : '';
		...
		if (empType == 'contractor') {
			if (!empty(pre_stub)) {
				...
				output['tytd'] = REGEXPATTERN.number_format(paymentTotal * month * tdiff, 2, '.', '');
				output['ytd1'] = REGEXPATTERN.number_format(current1 * month * tdiff, 2, '.', '');
				output['ytd2'] = REGEXPATTERN.number_format(current2 * month * tdiff, 2, '.', '');
				output['ytd3'] = REGEXPATTERN.number_format(current3 * month * tdiff, 2, '.', '');
				output['ytd4'] = REGEXPATTERN.number_format(current4 * month * tdiff, 2, '.', '');
				output['ytd5'] = REGEXPATTERN.number_format(current5 * month * tdiff, 2, '.', '');
